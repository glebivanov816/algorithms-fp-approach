module Exercises.Five.Eight.Heap (
  Heap,
  emptyHeap,
  heapEmpty,
  findHeap,
...
insertHeap newValue EmptyHp = Hp newValue 1 EmptyHp EmptyHp
insertHeap newValue (Hp value rank left right)
  | newValue > value = Hp value rank (insertHeap newValue left) right
  | otherwise = makeHeap newValue left right
